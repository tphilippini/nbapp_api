...
deviceController.patch = (req, res) => {
  ...
  const checking = () => {
    ...
    if (!revoked && !name) {
      ...
      Devices.doesTheDeviceBelongToTheUser(
        [req.params.uuid, req.user.user, req.user.user_type],
        (result) => {
          if (result) {
  ...
  checkEvent.on('success_revoked', () => {
    Devices.updateOneColumn(
      ['revoked', 1, req.params.uuid, req.user.user, req.user.user_type],
      () => {
  ...
  checkEvent.on('success_name', () => {
    Devices.updateOneColumn(
      ['name', name, req.params.uuid, req.user.user, req.user.user_type],
      () => {
